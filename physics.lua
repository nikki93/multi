        ...
        "serverSyncs",
        {
            ...
            receiver = function(game, time, tickCount, worldId, syncs, ownerships)
                ...
                for id, sync in pairs(syncs) do
                    local obj = self.idToObject[id]
                    if
                        obj and self.objectDatas[obj].ownerId ~= self.game.clientId and
                            ownerships[id] ~= self.game.clientId
                     then
...
function Physics:updateWorld(worldId, dt)
    ...
    if
        self.game.client and
            (worldData.lastServerSyncTime and self.game.time - worldData.lastServerSyncTime > 5 / self.serverSyncsRate)
     then
